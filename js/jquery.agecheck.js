  ...
  $.ageCheck = function (options) {
    ...
    const _this = {
      ...
      reCenter(b) {
        b.css(
          "top",
          `${Math.max(0, ($(window).height() - (b.outerHeight() + 150)) / 2)}px`
        );
        b.css(
          "left",
          `${Math.max(0, ($(window).width() - b.outerWidth()) / 2)}px`
        );
      },
      buildHtml() {
        ...
        html +=
          `<p>${copy.replace("[21]", `<strong>${settings.minAge}</strong>`)}` +
          "</p>";
      ...
      getStorage() {
        if (settings.storage === "cookie") {
          return document.cookie
            .split(";")
            .filter((item) => item.trim().startsWith("ageVerified=")).length;
        } else {
    ...
    $(".ac-container button").on("click", () => {
      ...
      if (_this.validate() === true) {
        ...
        if (_this.age >= settings.minAge) {
          if (
            !_this.setStorage("ageVerified", "true", settings.storageExpires)
          ) {
